...
export async function createGalleryItem(data: FormData) {
  try {
    ...
    await logUserActivity(
      userId,
      "create_gallery_item",
      `Created gallery item with id=${created.id} and title="${created.title}"`,
      { galleryItemId: created.id }
    );

    revalidatePath("/cms/gallery");

  return { ok: true, message: `Created: ${created.title}` };
  } catch (err: any) {
    console.error("createGalleryItem error", err);
    return { ok: false, message: err?.message || "Failed to create gallery item" };
  }
}
...
export async function updateGalleryItem(id: string, data: FormData) {
  try {
    ...
    await logUserActivity(
      userId,
      "update_gallery_item",
      `Updated gallery item id=${id} with title="${data.get("title")}"`,
      { galleryItemId: id }
    );

    revalidatePath("/cms/gallery");

  return { ok: true, message: `Updated: ${data.get("title") as string}` };
  } catch (err: any) {
    console.error("updateGalleryItem error", err);
    return { ok: false, message: err?.message || "Failed to update gallery item" };
  }
}
...
export async function deleteGalleryItem(id: string) {
  try {
    ...
    await logUserActivity(
      userId,
      "delete_gallery_item",
      `Deleted gallery item with id=${id}`,
      { galleryItemId: id }
    );

    revalidatePath("/cms/gallery");

  return { ok: true, message: `Deleted gallery item` };
  } catch (err: any) {
    console.error("deleteGalleryItem error", err);
    return { ok: false, message: err?.message || "Failed to delete gallery item" };
  }
}
